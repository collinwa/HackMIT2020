...
async function retrieveUser(userId) {
    ...
    try {
        await client.connect();
        console.log("retrieveUser: connected correctly to server");
        const db = client.db(dbName);
        const col = db.collection(colName);
...
async function updateUser(userId, newParam) {
    ...
    try {
        ...
        const db = client.db(dbName);
        const col = db.collection(colName);
        var currentChannelPoints = (await col.findOne({name: userId}))
        if (currentChannelPoints == null) {
          await col.insertOne({name: userId, channelPoints: newParam});
        } else {
          await col.updateOne({name: userId}, {$set: {channelPoints: currentChannelPoints.channelPoints + newParam}}, {upsert: true});
        }
    }
    catch(err) {
        console.log(err);
...
async function deleteUser(query) {
    ...
    try {
        await client.connect();
        console.log("deleteUser: Connected correctly to server");
        const db = client.db(dbName);
        const col = db.collection(colName);
...
async function deleteManyByQuery(query) {
    ...
    try {
        await client.connect();
        console.log("deleteManyByQuery: connected correctly to server");
        const db = client.db(dbName);
        const col = db.collection(colName);
